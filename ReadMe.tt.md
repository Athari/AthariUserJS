# User JS

[ [English](/ReadMe.md) | [Русский](/ReadMe.ru.md) | [Українська](/ReadMe.uk.md) | [Беларуская](/ReadMe.be.md) | [Български](/ReadMe.bg.md) | [**Татарча**](/ReadMe.tt.md) | [Slovenščina](/ReadMe.sl.md) | [Srpski](/ReadMe.sr.md) | [ქართული](/ReadMe.ka.md) ]

[@Athari](/Athari) тарафыннан ясалган **кулланучы скриптлары** җыентыгы.

* [Animator.ru](#animator): Чын Сылтамалар
* [Culture.ru](#culture): Ачылган
* [Kinorium.com](#kinorium): Яхшыртылган
* [Premier.one](#premier): Яхшыртылган
* [Берничә сайт өчен](#multiple)
  * [ReYohoho](#reyohoho): Рекламасыз + Яхшыртулар

**Карагыз: [User CSS](https://github.com/Athari/AthariUserCSS)** (@Athari'ның кулланучы стильләре җыентыгы).

## ⚙️ Куллану күрсәтмәләре

1. **🐵 кулланучы скриптлары өчен киңәйтүне урнаштырыгыз** (берсен сайлагыз):

   * <img src="https://www.tampermonkey.net/images/ape.svg" height=16> [Tampermonkey](https://www.tampermonkey.net/) өчен
     [Chrome](https://chromewebstore.google.com/detail/dhdgffkkebhmkfjojejmpbldmpobfkfo) /
     [Edge](https://microsoftedge.microsoft.com/addons/detail/iikmkjmpaadaobahmlepeloendndfphd) /
     [Firefox](https://addons.mozilla.org/tt/firefox/addon/tampermonkey/) /
     [Safari](https://apps.apple.com/app/tampermonkey/id6738342400)  
     (хәзерге вакытта ябык чыганаклы, иң күп функцияле)

   * ![][vm-logo] [Violentmonkey](https://violentmonkey.github.io/) өчен
     [Chrome](https://chrome.google.com/webstore/detail/violent-monkey/jinjaccalgkegednnccohejagnlnfdag) /
     [Edge](https://microsoftedge.microsoft.com/addons/detail/eeagobfjdenkkddmbclomhiblgggliao) /
     [Firefox](https://addons.mozilla.org/tt/firefox/addon/violentmonkey/)  
     (ачык чыганаклы)

   * <img src="https://addons.mozilla.org/user-media/addon_icons/0/748-32.png" height=16> [<del>Greasemonkey</del>](https://www.greasespot.net/) өчен
     [Firefox](https://addons.mozilla.org/tt/firefox/addon/greasemonkey/)  
     (ачык чыганаклы, минималистик, искергән)

2. **Киңәйтү төймәсен инструментлар панеленә урнаштырырга киңәш ителә**, чөнки бу урнаштырылган кулланучы скриптларына һәм аларның параметрларына керү мөмкинлеге бирәчәк.

3. Түбәндәге кулланучы скриптына күчегез һәм **аның ![User Script Install][Install Script] билгесенә басыгыз**. Скриптның карап чыгу тәрәзәсе ачылачак.

4. **Скриптны урнаштыру өчен өске панельдәге ![Install][Install Script Monkey] төймәсенә басыгыз**.

5. Әгәр дә сез берәр вакытта **скриптны сүндерергә яки яңадан кабызырга теләсәгез**, киңәйтү төймәсенә <img src="https://addons.mozilla.org/user-media/addon_icons/0/748-32.png" height=16> басыгыз, аннары скрипт янындагы күчергечкә басыгыз.

## <a id="animator"/> ![](https://icons.duckduckgo.com/ip3/animator.ru.ico) Animator.ru

### Animator.ru – Чын Сылтамалар [![Animator.ru Чын Сылтамалар Скриптын Урнаштырырга][Install Script]](Animator/Animator-TrueLinks.user.js?raw=1)

Animator.ru сайтында навигацияне яхшырта:

* Озын исемлекләрне битләргә бүлмичә күрсәтә.
* Скрипт сылтамаларын (JavaScript) чын сылтамаларга (href) алыштыра, шуңа күрә аларны яңа вкладкада ачарга, адресны күчерергә һ.б. мөмкин.

## <a id="culture"/> ![](https://www.google.com/s2/favicons?sz=32&domain=culture.ru) Culture.ru

### Culture.ru – Ачылган [![Culture.ru Ачылган Скриптын Урнаштырырга][Install Script]](Culture/Culture-Unlocked.user.js?raw=1)

"Видеозапись недоступна для просмотра по решению правообладателя" хатасын китереп чыгара торган хатаны төзәтә.

* Хостингта турыдан-туры урнаштырылган видеоларны һәрвакыт күрсәтә.
* Рәсем галереясе миниатюраларының зурлыгын көйләргә мөмкинлек бирә.

## <a id="kinorium"/> ![](https://icons.duckduckgo.com/ip3/kinorium.com.ico) Kinorium.com

### Kinorium.com – Яхшыртылган [![Kinorium.com Яхшыртылган Скриптын Урнаштырырга][Install Script]](Kinorium/Kinorium-Enhanced.user.js?raw=1)

Төрле яхшыртулар:

* Кулланучы коллекцияләре белән эшләү уңайлыгы
* Өстәмә стриминг провайдерларына сылтамалар
* Рәсемнәрнең натив "ялкау" йөкләнеше
* һ.б.

Бу кулланучы скрипты ["Kinorium.com – Яхшыртылган" кулланучы стиле](https://github.com/Athari/AthariUserCSS#kinorium) белән берлектә куллану өчен эшләнгән [![Kinorium.com Яхшыртылган Стилен Урнаштырырга][Install Style]](https://github.com/Athari/AthariUserCSS/blob/master/Kinorium/Kinorium-Enhanced.user.css?raw=1).

## <a id="premier"/> ![](https://icons.duckduckgo.com/ip3/premier.one.ico) Premier.one

### Premier.one – Яхшыртылган [![Premier.one Яхшыртылган Скриптын Урнаштырырга][Install Script]](Premier/Premier-Enhanced.user.js?raw=1)

Төрле яхшыртулар һәм хаталарны төзәтү:

* 1969 елга кадәр чыккан фильмнарны фильтрлау төзәтелде.
* 10 еллык диапазоннар урынына 5 еллык диапазоннар кулланылды.
* Слайдерлар урынына тулысынча ачылган эпизод исемлекләре.
* Фильм исемлекләрендә IMDB һәм Кинопоиск рейтинглары.
* Эпизодларга туры сылтамалар.
* Видео уйнаткычта тулы эпизод исемнәре.
* Өстәмә фильтрлар (Premier'да Союзмультфильмнан башка карарлык әйбер юк).

Бу кулланучы скрипты ["Premier.one – Яхшыртылган" кулланучы стиле](https://github.com/Athari/AthariUserCSS#premier) белән берлектә куллану өчен эшләнгән [![Premier.one Яхшыртылган Стилен Урнаштырырга][Install Style]](https://github.com/Athari/AthariUserCSS/blob/master/Premier/Premier-Enhanced.user.css?raw=1).

## <a id="multiple"/> Берничә сайт өчен

### <a id="reyohoho"/> ![](https://reyohoho.github.io/reyohoho/icons/favicon-32x32.png) ReYohoho – Рекламасыз + Яхшыртулар [![ReYohoho Рекламасыз + Яхшыртулар Скриптын Урнаштырырга][Install Script]](Multiple/ReYohoho-NoAds.user.js?raw=1)

ReYohoho онлайн стриминг сервисының видео балансерларыннан реклама роликларын бетерә: Alloha, Collaps, HDVB, Lumex, Rezka, Turbo, Vibix, VideoCDN, VideoSeed һ.б.

Шулай ук мөмкин булганда кечкенә яхшыртулар куллана: өстәмә уйнату тизлекләре, дизайн көйләүләре һ.б.

[vm-logo]: data:image/png;base64,
[Install Script]: https://img.shields.io/badge/User%20Script-Install-brightgreen?logo=data:image/png;base64,
[Install Script Monkey]: https://img.shields.io/badge/Install-666?labelColor=444&logo=data:image/png;base64,
[Install Style]: https://img.shields.io/badge/User%20Style-Install-brightgreen?logo=data:image/png;base64,