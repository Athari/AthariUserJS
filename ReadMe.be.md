# User JS

[ [English](/ReadMe.md) | [Русский](/ReadMe.ru.md) | [Українська](/ReadMe.uk.md) | [**Беларуская**](/ReadMe.be.md) | [Български](/ReadMe.bg.md) | [Татарча](/ReadMe.tt.md) | [Slovenščina](/ReadMe.sl.md) | [Srpski](/ReadMe.sr.md) | [ქართული](/ReadMe.ka.md) ]

Калекцыя **карыстальніцкіх скрыптоў** [@Athari](/Athari).

* [Animator.ru](#animator): Сапраўдныя Спасылкі
* [Culture.ru](#culture): Адкрыты
* [Kinorium.com](#kinorium): Палепшаны
* [Premier.one](#premier): Палепшаны
* [Для некалькіх сайтаў](#multiple)
  * [ReYohoho](#reyohoho): Без Рэкламы + Палепшанні

**Глядзіце таксама: [User CSS](https://github.com/Athari/AthariUserCSS)** (калекцыя карыстальніцкіх стыляў @Athari).

## ⚙️ Інструкцыя

1. **Усталюйце 🐵 пашырэнне для карыстальніцкіх скрыптоў** (выберыце адно):

   * <img src="https://www.tampermonkey.net/images/ape.svg" height=16> [Tampermonkey](https://www.tampermonkey.net/) для
     [Chrome](https://chromewebstore.google.com/detail/dhdgffkkebhmkfjojejmpbldmpobfkfo) /
     [Edge](https://microsoftedge.microsoft.com/addons/detail/iikmkjmpaadaobahmlepeloendndfphd) /
     [Firefox](https://addons.mozilla.org/be/firefox/addon/tampermonkey/) /
     [Safari](https://apps.apple.com/app/tampermonkey/id6738342400)  
     (зараз закрыты зыходны код, найбольш функцыянальнае)

   * ![][vm-logo] [Violentmonkey](https://violentmonkey.github.io/) для
     [Chrome](https://chrome.google.com/webstore/detail/violent-monkey/jinjaccalgkegednnccohejagnlnfdag) /
     [Edge](https://microsoftedge.microsoft.com/addons/detail/eeagobfjdenkkddmbclomhiblgggliao) /
     [Firefox](https://addons.mozilla.org/be/firefox/addon/violentmonkey/)  
     (адкрыты зыходны код)

   * <img src="https://addons.mozilla.org/user-media/addon_icons/0/748-32.png" height=16> [<del>Greasemonkey</del>](https://www.greasespot.net/) для
     [Firefox](https://addons.mozilla.org/be/firefox/addon/greasemonkey/)  
     (адкрыты зыходны код, мінімалістычнае, састарэлае)

2. Рэкамендуецца **прыкласці кнопку пашырэння на панэлі інструментаў**, каб атрымаць доступ да ўсталяваных карыстальніцкіх скрыптоў і іх налад.

3. Перайдзіце да карыстальніцкага скрыпта ніжэй і **націсніце на яго значок ![User Script Install][Install Script]**. Адкрыецца папярэдні прагляд скрыпта.

4. **Націсніце на кнопку ![Install][Install Script Monkey]** у верхняй панэлі, каб усталяваць скрыпт.

5. Калі вы захочаце **адключыць або зноў уключыць скрыпт**, націсніце на кнопку пашырэння <img src="https://addons.mozilla.org/user-media/addon_icons/0/748-32.png" height=16>, затым пераключыце тумблер побач са скрыптам.

## <a id="animator"/> ![](https://icons.duckduckgo.com/ip3/animator.ru.ico) Animator.ru

### Animator.ru – Сапраўдныя Спасылкі [![Усталяваць скрыпт Animator.ru Сапраўдныя Спасылкі][Install Script]](Animator/Animator-TrueLinks.user.js?raw=1)

Палепшае навігацыю на Animator.ru:

* Адлюстроўвае доўгія спісы цалкам без разбіцця на старонкі.
* Замяняе скрыптавыя спасылкі (JavaScript) на сапраўдныя (href), каб іх можна было адкрыць у новай укладцы, скапіраваць адрас і г.д.

## <a id="culture"/> ![](https://www.google.com/s2/favicons?sz=32&domain=culture.ru) Culture.ru

### Culture.ru – Адкрыты [![Усталяваць скрыпт Culture.ru Адкрыты][Install Script]](Culture/Culture-Unlocked.user.js?raw=1)

Выпраўляе памылку, якая прыводзіць да з'яўлення паведамлення "Видеозапись недоступна для просмотра по решению правообладателя".

* Заўсёды адлюстроўвае відэа (акрамя знешніх).
* Наладжвальны памер мініяцюр у галерэі малюнкаў.

## <a id="kinorium"/> ![](https://icons.duckduckgo.com/ip3/kinorium.com.ico) Kinorium.com

### Kinorium.com – Палепшаны [![Усталяваць скрыпт Kinorium.com Палепшаны][Install Script]](Kinorium/Kinorium-Enhanced.user.js?raw=1)

Розныя паляпшэнні:

* Зручнасць працы з карыстальніцкімі калекцыямі
* Спасылкі на дадатковыя анлайн-кінатэатры
* Натыўная лянівая загрузка малюнкаў
* і г.д.

Гэты карыстальніцкі скрыпт прызначаны для сумеснага выкарыстання з [карыстальніцкім стылем "Kinorium.com – Палепшаны"](https://github.com/Athari/AthariUserCSS#kinorium) [![Усталяваць стыль Kinorium.com Палепшаны][Install Style]](https://github.com/Athari/AthariUserCSS/blob/master/Kinorium/Kinorium-Enhanced.user.css?raw=1).

## <a id="premier"/> ![](https://icons.duckduckgo.com/ip3/premier.one.ico) Premier.one

### Premier.one – Палепшаны [![Усталяваць скрыпт Premier.one Палепшаны][Install Script]](Premier/Premier-Enhanced.user.js?raw=1)

Розныя паляпшэнні і выпраўленні памылак:

* Выпраўлена фільтрацыя фільмаў да 1969 г. (замест фільтрацыі фільмаў за толькі 1969 г.)
* Дыяпазоны гадоў у фільтрах зменены на 5-гадовыя (замест 10-гадовых).
* Спісы цалкам разгорнуты (замест палосак на 50 пунктаў без паўзункоў).
* Рэйтынгі IMDB і Кінапошуку ў спісах фільмаў (замест рэйтынгу Premier).
* Прамыя спасылкі на эпізоды і нядаўна праглядавыя фільмы (можна лёгка адкрыць у новай укладцы).
* Поўныя назвы эпізодаў у загалоўку відэаплэера (замест "сезон 1").
* Дадатковыя фільтры (акрамя Саюзмультфільма глядзець на Premier няма чаго).
* і г.д.

Гэты карыстальніцкі скрыпт прызначаны для сумеснага выкарыстання з [карыстальніцкім стылем "Premier.one – Палепшаны"](https://github.com/Athari/AthariUserCSS#premier) [![Усталяваць стыль Premier.one Палепшаны][Install Style]](https://github.com/Athari/AthariUserCSS/blob/master/Premier/Premier-Enhanced.user.css?raw=1).

## <a id="multiple"/> Для некалькіх сайтаў

### <a id="reyohoho"/> ![](https://reyohoho.github.io/reyohoho/icons/favicon-32x32.png) ReYohoho – Без Рэкламы + Палепшанні [![Усталяваць скрыпт Без Рэкламы + Палепшанні][Install Script]](Multiple/ReYohoho-NoAds.user.js?raw=1)

Выдаляе рэкламныя ролікі з відэабалансераў у анлайн-кінатэатры ReYohoho: Alloha, Collaps, HDVB, Lumex, Rezka, Turbo, Vibix, VideoCDN, VideoSeed і г.д.

Таксама па магчымасці прымяняе невялікія паляпшэнні: дадатковыя хуткасці прайгравання, выпраўленні дызайну і г.д.

[vm-logo]: data:image/png;base64,
[Install Script]: https://img.shields.io/badge/User%20Script-Install-brightgreen?logo=data:image/png;base64,
[Install Script Monkey]: https://img.shields.io/badge/Install-666?labelColor=444&logo=data:image/png;base64,
[Install Style]: https://img.shields.io/badge/User%20Style-Install-brightgreen?logo=data:image/png;base64,